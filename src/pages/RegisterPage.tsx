...
import { ArrowLeft, Building, MapPin, Phone, Camera, Check } from 'lucide-react';
...
const RegisterPage: React.FC = () => {
  ...
  const generateBeverCode = () => {
    const wardIndex = wardsData.wards.findIndex(w => w.name === formData.ward) + 1;
    const wardCode = wardIndex.toString().padStart(2, '0');
    const categoryCode = formData.customerCategory === 'Office' ? 'OF' : 
                        formData.customerCategory === 'Provision Store' ? 'PS' : 'HM';
    const customerId = Math.floor(Math.random() * 9999) + 1;
    return `JAL-${wardCode}-${categoryCode}-${customerId.toString().padStart(4, '0')}`;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const newUser = {
        ...
        customerCategory: formData.customerCategory as 'Office' | 'Provision Store' | 'Home',
        phoneNumber: formData.phoneNumber,
        beverCode,
        verificationStatus: 'Pending' as const,
        shopPhoto: formData.shopPhoto ? URL.createObjectURL(formData.shopPhoto) : undefined,
        createdAt: new Date(),
        hasPin: false
      };
  ...
  const isStepValid = () => {
    switch (currentStep) {
      case 1:
        return formData.businessName && formData.businessAddress && formData.ward && formData.customerCategory && formData.phoneNumber;
      case 2:
      ...
      <div className="px-6 py-6">
        <AnimatePresence mode="wait">
          ...
          {currentStep === 1 && (
            ...
            >
              <div>
                <h2 className="text-2xl font-bold text-primary-950 mb-2">Business Information</h2>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {['Office', 'Provision Store', 'Home'].map(category => (
                      <button
                        key={category}
                        type="button"
                        onClick={() => setFormData(prev => ({ ...prev, customerCategory: category }))}
          ...
          {currentStep === 2 && (
            ...
            >
              <div>
                <h2 className="text-2xl font-bold text-primary-950 mb-2">Shop Photo</h2>
                <p className="text-primary-600">Upload a clear photo of your shop's entrance or facade</p>
              </div>
              ...
              <div className="space-y-4">
                <div className="border-2 border-dashed border-primary-200 rounded-2xl p-8 text-center">
                  {formData.shopPhoto ? (
                    <div className="space-y-4">
                      ...
                      <div>
                        <p className="font-medium text-primary-950">Photo uploaded successfully</p>
                        <p className="text-sm text-primary-600">{formData.shopPhoto.name}</p>
                      </div>
                      <button
                        type="button"
                        onClick={() => setFormData(prev => ({ ...prev, shopPhoto: null }))}
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        <p className="font-medium text-primary-950 mb-2">Upload Shop Photo</p>
                        <p className="text-sm text-primary-600 mb-4">
                          Take a clear photo showing your shop's building or entrance
                        </p>
                ...
                <div className="bg-accent-50 border border-accent-200 rounded-xl p-4">
                  <h4 className="font-medium text-accent-800 mb-2">Photo Guidelines:</h4>
          ...
          {currentStep === 3 && (
            ...
            >
              <div>
                <h2 className="text-2xl font-bold text-primary-950 mb-2">Review Information</h2>
                <p className="text-primary-600">Please review your information before submitting</p>
              </div>

              <div className="card space-y-4">
                <div>
                  <h3 className="font-medium text-primary-950 mb-3">Business Details</h3>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-primary-600">Business Name:</span>
                      <span className="text-primary-950 font-medium">{formData.businessName}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-primary-600">Category:</span>
                      <span className="text-primary-950 font-medium">{formData.customerCategory}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-primary-600">Ward:</span>
                      <span className="text-primary-950 font-medium">{formData.ward}</span>
                    </div>
                    {formData.subArea && (
                      <div className="flex justify-between">
                        <span className="text-primary-600">Sub Area:</span>
                        <span className="text-primary-950 font-medium">{formData.subArea}</span>
                      </div>
                    )}
                    <div className="flex justify-between">
                      <span className="text-primary-600">Phone:</span>
                      <span className="text-primary-950 font-medium">{formData.phoneNumber}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="border-t border-primary-100 pt-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-accent-600 font-medium">✓ Uploaded</span>
                  </div>
                </div>
              </div>

              <div className="bg-primary-50 border border-primary-200 rounded-xl p-4">
                <h4 className="font-medium text-primary-950 mb-2">What happens next?</h4>
